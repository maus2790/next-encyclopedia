...
export default async function ConceptoPage({ params }: ConceptoPageProps) {
  ...
  const difficultyColors = {
    básico: "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/30",
    intermedio: "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/30",
    avanzado: "bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-500/30",
  };

  return (
    <div className="container mx-auto py-10 px-4 sm:px-6 max-w-4xl space-y-8">
      {/* Breadcrumb Navigation */}
      <div className="flex items-center justify-between gap-4 text-xs text-slate-500 dark:text-zinc-400 border-b border-slate-200 dark:border-white/10 pb-4">
        <Link
          href="/conceptos"
          className="inline-flex items-center gap-1.5 hover:text-slate-900 dark:hover:text-white transition-colors group"
        >
          <ArrowLeft className="h-3.5 w-3.5 transition-transform group-hover:-translate-x-1" />
          <span>Volver al catálogo</span>
        </Link>
        <div className="flex items-center gap-2">
          <span className="text-slate-400 dark:text-zinc-500">Enciclopedia</span>
          <span>/</span>
          <Link
            href={`/conceptos?capa=${concepto.capa}`}
            className="hover:text-indigo-600 dark:hover:text-indigo-300 transition-colors font-medium"
            style={{ color: capa?.color }}
          >
            Capa {concepto.capaNumero}
          </Link>
          <span>/</span>
          <span className="text-slate-900 dark:text-white font-medium">{concepto.nombre}</span>
        </div>
      </div>

      {/* Main Card / Article */}
      <article className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-white/10 space-y-8 relative overflow-hidden">
        {/* Glow corner */}
        <div
        ...
        {concepto.resumen_practico && (
          <div className="p-4 sm:p-5 rounded-2xl bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-500/30 text-indigo-900 dark:text-indigo-200 space-y-2 shadow-xs">
        ...
        <div className="space-y-4">
          ...
          <div
            className="text-slate-700 dark:text-zinc-300 text-sm leading-relaxed space-y-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-1 [&_strong]:text-slate-900 dark:[&_strong]:text-white [&_code]:bg-slate-100 dark:[&_code]:bg-white/10 [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_code]:text-indigo-700 dark:[&_code]:text-indigo-300 [&_code]:font-mono"
        ...
        {/* AI Prompt Ready Box */}
        {concepto.prompt_ia && (
          <div className="p-5 rounded-2xl bg-purple-50 dark:bg-gradient-to-r dark:from-purple-950/50 dark:to-slate-900 border border-purple-200 dark:border-purple-500/30 space-y-3 shadow-xs">
            <div className="flex items-center justify-between gap-2">
              ...
              <Link
                href={`/prompt-studio`}
                className="text-xs text-purple-700 dark:text-purple-400 hover:underline flex items-center gap-1 font-medium"
              >
            ...
            <div className="p-3 rounded-xl bg-slate-900 text-purple-200 dark:bg-black/60 border border-slate-700 dark:border-white/5 font-mono text-xs select-all shadow-inner">
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-white/10 flex flex-wrap items-center gap-2">
          <span className="text-xs text-slate-500 dark:text-zinc-400 font-semibold flex items-center gap-1 mr-2">
          ...
          {concepto.tags.map((tag) => (
            <span
              key={tag}
              className="text-xs px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-700 dark:text-zinc-300 font-mono"
            >
              #{tag}
            </span>
          ))}
            ...
            {relacionadosConceptos.map((rel) => {
              ...
              return (
                <Link
                  ...
                  className="p-4 rounded-xl glass-card border border-slate-200 dark:border-white/10 hover:border-indigo-300 dark:hover:border-white/20 transition-all flex flex-col justify-between group shadow-xs"
                >
                  <div>
                    <span
                  ...
                  <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mt-3 flex items-center gap-1">
                    <span>Leer guía</span>
                    <ExternalLink className="h-3 w-3" />
                  </span>
                </Link>
