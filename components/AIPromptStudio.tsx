...
import { Concepto, getAllConceptos } from "@/lib/conceptos";
import { 
  Sparkles, 
  Copy, 
  Check, 
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="p-5 rounded-2xl bg-slate-900/80 border border-white/10 space-y-3">
            ...
            <select
              ...
              className="w-full p-2.5 rounded-xl bg-slate-950 border border-white/15 text-white text-sm focus:outline-none focus:border-indigo-500"
            >
              {conceptos.map((c) => (
                ...
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl text-xs font-medium transition-all flex items-center justify-between ${
                    selectedGoal === goal.id
                      ...
                      : "bg-slate-950/60 text-zinc-400 hover:text-white border border-white/5 hover:bg-slate-950"
                  }`}
                >
                  <span>{goal.label}</span>
                  {selectedGoal === goal.id && <Check className="h-3.5 w-3.5 text-indigo-400" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-slate-900/80 border border-white/10 space-y-2">
            ...
            <textarea
              ...
              className="w-full p-2.5 rounded-xl bg-slate-950 border border-white/15 text-white text-xs placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500 h-20 resize-none"
            />
          </div>
        </div>

        {/* Right: Generated Prompt Preview & Copy Action */}
        <div className="lg:col-span-7 glass-panel rounded-2xl p-6 border border-white/10 flex flex-col justify-between space-y-4">
          <div>
            ...
            <div className="p-4 rounded-xl bg-black/50 border border-white/10 font-mono text-xs text-zinc-300 whitespace-pre-wrap leading-relaxed max-h-[380px] overflow-y-auto select-all">
              {generatedPrompt}
            </div>
          </div>

          <div className="p-3.5 rounded-xl bg-purple-500/10 border border-purple-500/20 text-xs text-purple-300 flex items-center justify-between">
