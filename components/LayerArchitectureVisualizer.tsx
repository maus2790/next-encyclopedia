  ...
  return (
    <div className="w-full my-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
        ...
        <Link
          href="/conceptos"
          className="inline-flex items-center gap-2 text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors self-start md:self-end"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            return (
              <button
                ...
                className={`relative w-full text-left p-3.5 rounded-xl transition-all duration-300 flex items-center justify-between group ${
                  isSelected
                    ? "bg-white dark:bg-slate-900 border-2 shadow-lg ring-1"
                    : "bg-white/70 dark:bg-slate-950/60 hover:bg-white dark:hover:bg-slate-900/80 border border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/20"
                }`}
                style={{
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-200 transition-colors">
                        {capa.nombre.split("(")[0]}
                      </span>
                    </div>
                    <span className="text-[11px] text-slate-500 dark:text-zinc-400 block truncate max-w-[200px] sm:max-w-xs">
                      {capa.subtitulo}
                    </span>
                  </div>
                </div>
        ...
        <div className="lg:col-span-7 glass-panel rounded-2xl p-6 border border-slate-200 dark:border-white/10 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-600 dark:text-zinc-300 font-medium mt-1">
                  {activeLayer.subtitulo}
                </p>
              </div>
              <Link
                href={`/conceptos?capa=${activeLayer.id}`}
                className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-700 dark:text-white border border-slate-200 dark:border-white/15 bg-white dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors shrink-0 flex items-center gap-1 shadow-xs"
              >
            ...
            {/* Description */}
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 text-sm text-slate-700 dark:text-zinc-300 leading-relaxed">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[11px] text-slate-400 dark:text-zinc-500">Haz clic para leer la guía técnica</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {layerConceptos.map((concepto) => (
                  <Link
                    ...
                    className="p-3.5 rounded-xl bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-white/5 hover:border-indigo-300 dark:hover:border-white/20 hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-all group flex flex-col justify-between shadow-xs hover:shadow-md"
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        <span className="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors">
                          {concepto.nombre}
                        </span>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-zinc-400 border border-slate-200 dark:border-white/5">
                          {concepto.dificultad}
                        </span>
                      </div>
                    ...
                    {concepto.resumen_practico && (
                      <div className="mt-2 pt-2 border-t border-slate-100 dark:border-white/5 text-[11px] text-indigo-600 dark:text-indigo-300/90 flex items-center gap-1">
